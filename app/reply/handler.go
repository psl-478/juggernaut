...
func ReplyHandler(c *gin.Context) {
	...
	case *Reply:
		// 处理Reply对象
		// 判断客服处理人在配置成员中是否存在
		msg.(*Reply).Assignee = pkg.ExtractChineseNameUnicode(msg.(*Reply).Assignee) //对象断言后修改值20251106
	...
	case *Silences:
		msg.(*Silences).Assignee = pkg.ExtractChineseNameUnicode(msg.(*Silences).Assignee) //对象断言后修改值20251106
		// 处理Silences对象
		//发送到对应agent的静默通道中
...
func (r *Reply) checkAsignee(udeskId string) (bool, string) {
	...
	body, _ := io.ReadAll(resp.Body)
	jsonData := string(body)
	assignee := pkg.ExtractChineseNameUnicode(gjson.Get(jsonData, "ticket.assignee_name").String()) //调用接口改成不带英文名的类型20251106
