...
(____/   (_______)(_______)(_______)(_______/|/   \__/|/    )_)|/     \|(_______)   )_(
	...
	r.LoadHTMLFiles("./html/index.html", "./html/contributors.html", "./html/login.html", "./html/rota.html", "./html/game.html")
	...
	{ //验证码转发路由
		...
		backend.GET("/login", logMiddleware(readMe), read.Login)
		backend.GET("/rota", logMiddleware(readMe), read.Rota)
		backend.GET("/game", logMiddleware(oneword), read.Game)
		...
		rota := backend.Group("/rota")
		{ //获取值班表
			rota.GET("/get_rota", logMiddleware(duty), stand.Data)
			//更新值班表
			rota.POST("/update_rota", logMiddleware(duty), stand.Update)
			//上传值班表
			rota.POST("/upload_rota", logMiddleware(duty), stand.Upload)
		}
	}
	return r
}
